...
class Calendar {
    ...
    protected function _dlog($msg){
    	$this->modx->log(modx::LOG_LEVEL_ERROR,print_r($msg,true));
    }
    ...
    public function isAdmin(){
        ...
        // edited so that only certain people can alter the system through the built in interface
    ...
    public function process($scriptProperties){
        ...
        if ( $this->show_calendar ){
			...
			//jw - removing multiple views for now.  Month is sufficient for this use case.
            //$html .= $this->calToggle();            
            $f = $this->displayFilters();
			
			$html = $this->modx->getChunk($this->filters['calMaster'],array('calendarFilter'=>$f, 'calendarWithNav'=>$html));
    ...
    public function getFilters($scriptProperties=array()){
        ...
        if ($this->filters['view'] == 'list' ) {
            ...
            $this->filters['filterLocations'] =  
            	(in_array(0,$this->filters['locations'])==1||count($this->filters['locations'])==0)?0:1;			
        ...
        $this->filters['calToggleTpl'] = $this->modx->getOption('calToggleTpl', $scriptProperties, $skin.'_calToggleTpl');
        ...
        $this->filters['calMaster'] = $this->modx->getOption('calMaster', $scriptProperties, $skin.'_calMaster');
        $this->filters['regionSelection'] = $this->modx->getOption('regionSelection', $scriptProperties, $skin.'_regionSelection');
     }
    /**
    ...
    public function displayFilters(){
        ...
        if ( $this->getFilter('useLocations') ) {
            ...
            foreach ($locationTypes as $locationType) {
                ...
                foreach ($locations as $location) {
                    ...
                    $locProperties['selected'] = in_array($location->get('id'),$this->filters['locations']) ? 'selected="selected"' : '';
        ...
        $properties['allLocationSelected'] = in_array(0,$this->filters['locations']) || count($this->filters['locations'])==0 ? 'selected="selected"': '';
        $html = $this->getChunk($this->filters['calFilterTpl'], $properties);
        return $html;
    }
    
    public function calToggle(){
		$properties = array(
		   'cDay' => date("j"),
		   'cMonth' => date("n"),
		   'cYear' => date("Y"),
		...
		$html = $this->getChunk($this->filters['calToggleTpl'], $properties);
		return $html;  
    }
    ...
    public function loadHead(){
        ...
        $properties = array_merge($properties, array('westtn'=>$this->unpackRegion($this->filters['regionSelection'][0])));
        $properties = array_merge($properties, array('middletn'=>$this->unpackRegion($this->filters['regionSelection'][1])));
        $properties = array_merge($properties, array('easttn'=>$this->unpackRegion($this->filters['regionSelection'][2])));
    ...
    public function unpackRegion($arrRegion){
		$return='';
    ...
    public function loadEvents($view=NULL){
        ...
        if ( $this->filters['filterLocations']==1 && $this->filters['useLocations']) {
...
ORDER BY ce.start_date ASC 
    ...
    public function getKCEvent($properties){

		
		//print_r($properties);
    	    	$mapIconPath = 'assets/images/util/mapicon.png';

    	$council=$this->getKCCouncil($properties['locationInfo']);
    	
    	if($council){
    	
    	$caQuery = $this->modx->newQuery('councilAddress',array('councilId'=>$council->get('id')));
    	$councilAddresses=$this->modx->getIterator('councilAddress',$caQuery);
    	
		foreach ($councilAddresses as $key => $value) {
			$thisAddress=$value->toArray();
			$myAddress=$myAddress?($thisAddress['addressType']=='physical'?$thisAddress:$myAddress):$thisAddress;
	    }    	
    	   ...
    	   case 'Second Degree':
       	   ...
       	   case 'Fourth Degree':
       			$html.='<span>Host Council: '.$council->get('councilNumber').' - '.$council->get('name').'</span><br/>';
       	   		...
       	   		$html.='<span>Candidates Arrive: '.$properties['office_notes'].' Minutes before</span><br/>'  ;
       	   		$html.="<span>Contact: <a href='mailto:".$properties['contact_email']."?subject=".$properties['event_title']."'>".$properties['contact']."</a></span><br/>";
       	   		break;
       	   case 'Event':
       	   case 'State Meeting':
       	   		if ($council){
   	    		...
   	    		$html.='<span>Location:'.$myAddress['city'].'</span><br/><a href=\'http://maps.google.com?q='.$mapLink.'\'>Click to see map:<img class=\'mapicon\' src=\''.$mapIconPath.'\' alt=\'Click to show map\'/></a><br/><br/>';
					...
					$html.='<span>Location:'.$properties['locationInfo'].'</span><br/><a href=\'http://maps.google.com?q='.urlencode($properties['locationInfo']).'\'>Click to see map:<img class=\'mapicon\' src=\''.$mapIconPath.'\' alt=\'Click to show map\'/></a><br/><br/>';				
				}
				if($properties['start_time']=='All day'){
					$html.='<span>All Day Event</span><br/>';
				} else {
   	    		$html.='<span>Start Time: '.date_format(date_create($properties['public_start']), 'g:i A').'</span><br/>';
   	    		$html.='<span>End Time: '.date_format(date_create($properties['end_date']), 'g:i A').'</span><br/>';
   	    		}
   	    		$html.="<span>Contact: <a href='mailto:".$properties['contact_email']."?subject=".$properties['event_title']."'>".$properties['contact']."</a></span><br/>";
           	   	break;
           case 'Important Deadline':
           default:
				break;
       	   
     	}
     	$html.='</div>';
     	return array ('event_summary'=>$html);
    	
    }
    
    public function getKCCouncil($locstring){

		$base_path = $this->modx->getOption('core_path').'components/kofctn/';
		//print_r($base_path);
		
		$this->modx->addPackage('kofctn',$base_path.'model/');

    	if ($locstring[0]=='#'){
    		$cnumber = substr($locstring,1,strpos($locstring,' '))+0;
    	}

		if ($cnumber){
			
			$q= $this->modx->newQuery('council');
			$q->where(array('councilNumber'=>$cnumber));
			/*$q->prepare();
			print_r($q->toSQL());
			*/
			$council = $this->modx->getObject('council',$q);
			return $council;
    ...
    public function selectOptions($options, $current, $first='All'){
        $html = '';
//        $this->modx->log(modX::LOG_LEVEL_ERROR,print_r($current,true));
        if ( !empty($first) ) {
            $html .= '<option value="0"';
            $html .= $current==0?' selected="selected"':'';
            $html .= '>'.$first.'</option>';
        }
