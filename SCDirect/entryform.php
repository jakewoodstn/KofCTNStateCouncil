...
if (($_COOKIE['validlogin']=='true')||($verifydat==true)){
	...
	if(isset($_COOKIE['speclogin'])){
		$speclogin = $_COOKIE['speclogin'];
	} else {
		$speclogin = "NO";
	}
	if(isset($_COOKIE['scdform'])){
...
/*		if ($assembly>0 || preg_match('/Faithful/',$position)){
		...
		if($err != ''){
			...
			switch($scdform){
			 	case "add":
					...
					$addstr = $_POST["lname"] .", " .$_POST["fname"] ." Mem Num:  " .$_POST["userid"];
					mail($createnotifyemail,"Record added", $addstr);
					break;
				case "update":
	...
	if (($logseclevel > 0) || ($scdform == 'edit')|| ($verifydat==true)) {
		include 'configdb.php';
		include 'opendb.php';
	...
	<form id="form1" name="form1" method="post"  action="entryform.php" class="niceform">
...
?>            
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button onclick="<?php echo "location.href='$locat'"; ?>">Cancel </button>
